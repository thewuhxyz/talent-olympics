...
use anchor_spl::{token_2022::spl_token_2022::extension::ExtensionType, token_interface::spl_token_2022::{
...
pub fn get_mint_space(extension_types: &[ExtensionType]) -> Result<(u64, u64)> {
    let space = ExtensionType::try_calculate_account_len::<Mint>(extension_types)?;
    let lamports = Rent::get()?.minimum_balance(space);

    Ok((u64::try_from(space)?,  lamports))
}